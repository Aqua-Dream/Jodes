...
class CurlClient
  ...
  public:
    ...
    virtual ~CurlClient() override {
        log_debug("CurlClient is being destroyed@@@@@@");
    }

    bool saveResponseToFile(const std::string& outputFilename);

    static proxygen::HTTPHeaders parseHeaders(const std::unordered_map<std::string, std::string>& headersString);
