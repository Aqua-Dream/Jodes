...
// DEFINE_int32(worker_id, -1, "worker id");
...
DEFINE_string(config, "../config/node.properties", "Path to config file, currently use the same file as coordinator");
...
int main(int argc, char* argv[]) {
    ...
    utils::is_distributed = true;
    log_info("http_port is %d", FLAGS_http_port);
 ...
 // {SocketAddress(FLAGS_ip, FLAGS_spdy_port, true), Protocol::SPDY },
  // {SocketAddress(FLAGS_ip, FLAGS_h2_port,   true), Protocol::HTTP2},
    ...
    if (initialize_enclave(false) < 0) {
        log_error("Error: enclave initialization failed");
    }
    log_info("enclave initialization succeed");

    // Start HTTPServer mainloop
    server.start();
    return 0;
}
