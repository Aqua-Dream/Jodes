...
cmake_minimum_required(VERSION 2.9) # target_include_directories
...
if(SGX_FOUND)
    ...
    set(SGX_COMMON_CFLAGS "${SGX_COMMON_CFLAGS} ${PROJECT_COMMON_CFLAGS}")

    set(ENCLAVE_INC_DIRS "${ENCLAVE_INC_DIRS}" "${SGX_INCLUDE_DIR}" "${SGX_TLIBC_INCLUDE_DIR}" "${SGX_LIBCXX_INCLUDE_DIR}")
    set(ENCLAVE_C_FLAGS "${SGX_COMMON_CFLAGS} -nostdinc -fvisibility=hidden -fpie -fstack-protector-strong -DENCLAVE_CONTEXT")
    set(ENCLAVE_CXX_FLAGS "${ENCLAVE_C_FLAGS} -nostdinc++")

    set(APP_INC_DIRS "${APP_INC_DIRS}" "${SGX_PATH}/include")
    ...
    function(add_enclave_library target)
        ...
        foreach(TLIB ${SGX_TRUSTED_LIBS})
            ...
            target_link_libraries(${target} ${TLIB})
        endforeach()
    ...
    function(add_untrusted_executable target)
        set(optionArgs USE_PREFIX)
        set(multiValueArgs SRCS EDL EDL_SEARCH_PATHS LINK_LIBS)
        cmake_parse_arguments("SGX" "${optionArgs}" "" "${multiValueArgs}" ${ARGN})
        if("${SGX_EDL}" STREQUAL "")
            message(FATAL_ERROR "${target}: SGX enclave edl file is not provided!")
        endif()
        ...
        set_target_properties(${target} PROPERTIES COMPILE_FLAGS ${APP_CXX_FLAGS})
        target_include_directories(${target} PRIVATE ${CMAKE_CURRENT_BINARY_DIR} ${APP_INC_DIRS})
        target_link_libraries(${target} ${SGX_LINK_LIBS} 
										 "${SGX_COMMON_CFLAGS} \
                                         ${APP_LINK_FLAGS} \
                                         -L${SGX_LIBRARY_PATH} \
                                         -l${SGX_URTS_LIB} \
